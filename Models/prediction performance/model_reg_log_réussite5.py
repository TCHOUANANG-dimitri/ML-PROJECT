...
class RegressionLogistique:
    ...
    def __init__ (self, learning_rate=0.01, n_iterations=1000): # déclare le constructeur comme étant une méthode privée
        self.lr = learning_rate       # permet de stocker le taux d'apprentissage. cette ligne sauvegarde learning_rate comme attribut de l'instance
        self.n_iterations = n_iterations
        self.weights = None  # initialise le poids à None de telle sorte qu'il soit apprit pendant l'entraînement
        self.bias = None 
        self.cost = []  # initialise une liste vide pour stocker l'évolution de la fonction coût à chaque itération
    
    def sigmoid(self, z):
        return 1 / (1 + np.exp(-z))
    
    def _add_bias(self, X):
        b = np.ones((X.shape[0], 1)) # crée une matrice unité ayant le même nbre de ligne que X et une colonne
        X = np.hstack((b, X)) # fait une conaténation horizontale de b et X
        return X 
    ...
    def fit(self, X, y): 
        ...
        X = self._add_bias(X)
        y = y.reshape(-1, 1) # redimensionne y de telle sorte que la dim des lignes est calculée automatiquement tandis que le nombre de colonne est 1
        self.weights = np.zeros((X.shape[1], 1))
        self.bias = 0 # Initialise le biais à 0
        self.cost = []
  
        for i in range(self.n_iterations): # Algorithme de descente du gradient
            self.weights -= self.lr * self._gradient(X, y) # mise à jour du poids par la descente du gradient
            self.cost.append(self._cost_function(X, y))
            # self._cost_function(X, y)` : Calcule la valeur actuelle de la fonction de coût (erreur du modèle) avec les poids mis à jour.
            # `.append()` : Ajoute cette valeur à la liste `self.cost` pour garder un historique de l'évolution du coût à chaque itération.
...
def train_test_split(X, y, test_size=0.2, random_state=None):
    ...
    test_indices = indices[:n_test] # extraire les n_test premiers éléments du tableau indices
...
model_charge = RegressionLogistique.load_json('modele_regression_logistique5.json')
